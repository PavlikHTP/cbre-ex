...
namespace CBRE.Editor
{
    public partial class Editor : HotkeyForm, IMediatorListener
    {
        ...
        public const string GITHUB_REPORT_BUG_URL =
            "https://github.com/AnalogFeelings/cbre-ex/issues/new?assignees=AnalogFeelings&labels=bug&template=bug_report.md&title=";
        ...
        private static void LoadFileGame(string fileName, Game game)
        {
            try
            {
                ...
                if (lightmaps != null)
                {
                    ...
                    foreach (Viewport3D viewport in ViewportManager.Viewports.Where(vp => vp is Viewport3D)
                                 .Select(vp => vp as Viewport3D))
                    {
                        viewport.Type = Viewport3D.ViewType.Lightmapped;
                        ViewportLabelListener listener =
                            viewport.Listeners.Find(l => l is ViewportLabelListener) as ViewportLabelListener;
                        listener?.Rebuild();
                    }
                }
            }
        ...
        private void EditorLoad(object sender, EventArgs e)
        {
            ...
            {
                BaseTool tl = tool;
                HotkeyImplementation hotkey =
                    CBRE.Settings.Hotkeys.GetHotkeyForMessage(HotkeysMediator.SwitchTool, tool.GetHotkeyToolType());
            ...
            Mediator.MediatorException += (mthd, ex) =>
                Logging.Logger.ShowException(ex.Exception, "Mediator Error: " + ex.Message);
        ...
        private async void CheckForUpdates(bool notFromMenu)
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                try
                {
                    ...
                    string serverResponse = await client.GetStringAsync(API_RELEASES_URL);

                    UpdaterResponse response = JsonConvert.DeserializeObject<UpdaterResponse>(serverResponse,
                        new JsonSerializerSettings
                        {
                            MissingMemberHandling = MissingMemberHandling.Ignore
                        });

                    if (response == null || !Version.TryParse(response.VersionTag, out Version parsedNewVersion))
                        return;
                    ...
                    {
                        var packageAsset = response.Assets.FirstOrDefault(x =>
                            x.Filename.EndsWith(".zip", StringComparison.OrdinalIgnoreCase));
                        var checksumAsset = response.Assets.FirstOrDefault(x =>
                            x.Filename.EndsWith(".sha256", StringComparison.OrdinalIgnoreCase));

                        if (packageAsset == null || checksumAsset == null)
                        {
                            if (MessageBox.Show(
                                    "There is a new update available, but the required files are missing. Open GitHub?",
                                    "Warning!", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                            {
                                Process.Start(GIT_LATEST_RELEASE_URL);
                            }

                            return;
                        }

                        using (UpdaterForm form = new UpdaterForm(parsedNewVersion, response.Description, packageAsset,
                                   checksumAsset))
                        {
                            form.ShowDialog();
                        }
        ...
        private bool PromptForChanges(Document doc)
        {
            ...
            {
                DialogResult result = MessageBox.Show("Would you like to save your changes to " + doc.MapFileName + "?",
                    "Changes Detected",
        ...
        protected override void OnDragDrop(DragEventArgs drgevent)
        {
            if (drgevent.Data.GetDataPresent(DataFormats.FileDrop))
            {
                ...
                List<string> files =
                    (drgevent.Data.GetData(DataFormats.FileDrop) as IEnumerable<string> ?? new string[0])
        ...
        private static void FileOpen()
        {
            using (OpenFileDialog ofd = new OpenFileDialog())
            {
                ...
                string[] all = FileTypeRegistration.GetSupportedExtensions().Where(x => x.CanLoad)
                    .Select(x => "*" + x.Extension).ToArray();
                ofd.Filter = "All supported formats (" + String.Join(", ", all) + ")|" + String.Join(";", all) + "|" +
                             filter;
        ...
        private void UpdateDocumentTabs()
        {
            ...
            else
            {
                for (int i = 0; i < DocumentManager.Documents.Count; i++)
                {
                    ...
                    DocumentTabs.TabPages[i].Text =
                        doc.MapFileName + (doc.History.TotalActionsSinceLastSave > 0 ? " *" : "");
                }
            }
        ...
        private void UpdateTitle()
        {
            if (DocumentManager.CurrentDocument != null)
            {
                ...
                Text =
                    $"{titleStart} - {(String.IsNullOrWhiteSpace(doc.MapFile) ? "Untitled" : System.IO.Path.GetFileName(doc.MapFile))}";
            }
            else
            {
                Text = titleStart;
        ...
        private void MouseCoordinatesChanged(Coordinate coord)
        {
            ...
            StatusCoordinatesLabel.Text =
                coord.X.ToString("0") + " " + coord.Y.ToString("0") + " " + coord.Z.ToString("0");
        }
        ...
        private void SelectionBoxChanged(Box box)
        {
            if (box == null || box.IsEmpty()) StatusBoxLabel.Text = "";
                ...
                StatusBoxLabel.Text = box.Width.ToString("0") + " x " + box.Length.ToString("0") + " x " +
        ...
        {
            ViewportBase focused = (parameter as ViewportBase) ??
                                   ViewportManager.Viewports.FirstOrDefault(x => x.IsFocused);
                       ...
                       .OkCancel())
            {
                ...
                Image shot =
                    ViewportManager.CreateScreenshot(focused, (int)qf.Decimal("Width"), (int)qf.Decimal("Height"));
                if (shot == null) return;

                string ext =
                    focused is Viewport2D || (focused is Viewport3D &&
                                              ((Viewport3D)focused).Type != Viewport3D.ViewType.Textured)
                        ? ".png"
                        : ".jpg";
                ...
                using (SaveFileDialog sfd = new SaveFileDialog())
                {
                    sfd.FileName = $"{titleStart} - "
                                   + (DocumentManager.CurrentDocument != null
                                       ? DocumentManager.CurrentDocument.MapFileName
                                       : "untitled")
        ...
        private void RecentFile(string path)
        {
            ...
            List<RecentFile> recents = SettingsManager.RecentFiles.OrderBy(x => x.Order).Where(x => x.Location != path)
                .Take(9).ToList();
